...
dashboardRoutes.get('/dashboard', async (c) => {
  try {
    ...
    const [vaccinesDue, upcomingEvents] = await Promise.all([
      query<{ cat_id: string; cat_name: string; vaccine_name: string; expiration_date: string; clinic_name: string | null }>(c.env.DB, `SELECT v.cat_id, c.name AS cat_name, v.vaccine_name, v.expiration_date, v.clinic_name FROM vaccinations v JOIN cats c ON v.cat_id = c.id WHERE date(v.expiration_date) <= date('now', '+30 days') ORDER BY v.expiration_date ASC`, []),
