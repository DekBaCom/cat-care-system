...
dashboardRoutes.get('/dashboard', async (c) => {
  try {
    ...
    const [urgentTasks, upcomingEvents] = await Promise.all([
      query<{ cat_name: string; vaccine_name: string; expiration_date: string }>(c.env.DB, `SELECT c.name AS cat_name, v.vaccine_name, v.expiration_date FROM vaccinations v JOIN cats c ON v.cat_id = c.id WHERE date(v.expiration_date) <= date('now', '+3 days') AND date(v.expiration_date) >= date('now') ORDER BY v.expiration_date ASC`, []),
