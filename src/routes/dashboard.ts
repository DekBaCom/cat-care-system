...
dashboardRoutes.get('/dashboard', async (c) => {
  try {
    ...
    const [totalCats, catsInTreatment, vaccinesExpiringSoon, activeMedications] = await Promise.all([
      ...
      queryOne<{ count: number }>(c.env.DB, `SELECT COUNT(*) AS count FROM vaccinations v WHERE v.expiration_date IS NOT NULL AND date(v.expiration_date) <= date('now', '+7 hours', '+30 days') AND v.id = (SELECT v2.id FROM vaccinations v2 WHERE v2.cat_id = v.cat_id AND v2.vaccine_name = v.vaccine_name ORDER BY v2.vaccination_date DESC, v2.created_at DESC LIMIT 1)`, []),
    ...
    const [vaccinesDue, upcomingEvents] = await Promise.all([
      query<{ cat_id: string; cat_name: string; vaccine_name: string; expiration_date: string; clinic_name: string | null }>(c.env.DB, `SELECT v.cat_id, c.name AS cat_name, v.vaccine_name, v.expiration_date, v.clinic_name FROM vaccinations v JOIN cats c ON v.cat_id = c.id WHERE v.expiration_date IS NOT NULL AND date(v.expiration_date) <= date('now', '+7 hours', '+30 days') AND v.id = (SELECT v2.id FROM vaccinations v2 WHERE v2.cat_id = v.cat_id AND v2.vaccine_name = v.vaccine_name ORDER BY v2.vaccination_date DESC, v2.created_at DESC LIMIT 1) ORDER BY v.expiration_date ASC`, []),
