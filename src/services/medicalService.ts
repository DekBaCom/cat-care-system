...
interface MedRow { id: string; cat_id: string; record_date: string; type: string; symptoms: string | null; diagnosis: string | null; clinic_name: string | null; veterinarian_name: string | null; treatment_date: string | null; cost: number | null; treatment_result: string | null; status: string; created_at: string; }
...
export class MedicalService {
  static async addMedicalRecord(catId: string, userId: string, data: Partial<MedicalHistory> & { status?: string }, env: Env): Promise<MedicalHistory> {
    await CatService.getCatById(catId, userId, env);
    const id = uuidv4(); const now = new Date().toISOString();
    const type = data.type ?? 'checkup';
    const status = data.status ?? (type === 'illness' || type === 'injury' || type === 'surgery' ? 'treating' : 'recovered');
    await execute(env.DB, `INSERT INTO medical_history (id, cat_id, record_date, type, symptoms, diagnosis, clinic_name, veterinarian_name, treatment_date, cost, treatment_result, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, catId, data.recordDate ?? now.slice(0, 10), type, data.symptoms ?? null, data.diagnosis ?? null, data.clinicName ?? null, data.veterinarianName ?? null, data.treatmentDate ?? null, data.cost ?? null, data.treatmentResult ?? null, status, now]);
    return rowToHistory((await queryOne<MedRow>(env.DB, 'SELECT * FROM medical_history WHERE id = ?', [id]))!);
  }

  static async updateStatus(id: string, catId: string, userId: string, status: string, env: Env): Promise<void> {
    await CatService.getCatById(catId, userId, env);
    if (!await queryOne<{ id: string }>(env.DB, 'SELECT id FROM medical_history WHERE id = ? AND cat_id = ?', [id, catId])) throw NOT_FOUND;
    await execute(env.DB, 'UPDATE medical_history SET status = ? WHERE id = ? AND cat_id = ?', [status, id, catId]);
  }
